...
router.get("/luggage-admin", verifyUser, async (req, res) => {
  try {
    ...
    if (!luggage) {
      return res
        .status(404)
        .json({ status: false, message: "Luggage not found" });
    }
    res.json(luggage);
  } catch (error) {
...
router.get("/fall-logs1", verifyUser, async (req, res) => {
  try {
    ...
    for (const luggage of luggageList) {
      const logs = await FallDetectionLog.find({
...
router.get("/tamper-logs1", verifyUser, async (req, res) => {
  try {
    ...
    const tamperLogs = [];
    for (const luggage of luggageList) {
      const logs = await TamperDetectionLog.find({
        luggage_tag_number: luggage.luggage_tag_number,
      });
...
router.get("/temp-logs", verifyUser, async (req, res) => {
  try {
    ...
    if (luggageList.length === 0) {
      return res
        .status(404)
        .json({ status: false, message: "No luggage found" });
    }
    const tempLogs = [];
    for (const luggage of luggageList) {
...
router.delete(
  "/delete-tracking-data/:tagNumber",
  verifyUser,
  async (req, res) => {
    ...
    try {
      const deletedFallData = await FallDetectionLog.deleteMany({
        luggage_tag_number: luggageTagNumber,
      });
      const deletedTamperData = await TamperDetectionLog.deleteMany({
        luggage_tag_number: luggageTagNumber,
      });
      const deletedTempData = await TempLog.deleteMany({
        luggage_tag_number: luggageTagNumber,
      });
      const deleteLocationdata = await Luggage.findOneAndUpdate(
        { luggage_tag_number: luggageTagNumber },
      ...
      if (
        ...
        deleteLocationdata.deletedCount === 0
      ) {
        return res
          .status(404)
          .json({ status: false, message: "Luggage not found" });
      ...
      return res.status(500).json({
        status: false,
        message: "Error deleting luggage tracking data",
      });
    }
  }
);
...
router.post("/luggage/:id/updateStatus", verifyUser, async (req, res) => {
  ...
  if (!status || !luggageId) {
    return res
      .status(400)
      .json({ status: false, message: "Status and luggageId are required" });
  }
...
router.get("/notifications", verifyUser, async (req, res) => {
  ...
  const sendUpdates = async () => {
    try {
      ...
      // luggage list associated with the user
      const luggageList = await Luggage.find({ user_id: user._id });
      if (luggageList.length === 0) {
        return res
          .status(404)
      ...
      console.error("Error fetching logs for SSE:", error);
      res.write(
